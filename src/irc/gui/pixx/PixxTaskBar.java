...
public class PixxTaskBar extends PixxPanel implements MouseListener,MouseMotionListener,BaseAWTSourceListener
{
  ...
  public void update(Graphics ug)
  {
    ...
    if(_buffer==null)
    {
      ...
      int sw=Math.max(g.getFontMetrics().stringWidth(getText(PixxTextProvider.GUI_PRIVATE)),g.getFontMetrics().stringWidth(getText(PixxTextProvider.GUI_PUBLIC)));
      _ileft=25+sw;
      computeWidth();
...
//      g.setColor(Color.white);
      ...
      g.drawString(getText(PixxTextProvider.GUI_PUBLIC),8,y+1);

      y=getY(1)+getItemHeight();
      y-=(getItemHeight()-_font.getSize())/2;

      g.drawString(getText(PixxTextProvider.GUI_PRIVATE),8,y-4);
  ...
  private TaskBarItem getItemAt(int x,int y)
  {
    ...
    //prentre le colième élément de la ligne row
    int currentcol=0;
    Enumeration el=_items.elements();
    while(el.hasMoreElements())
    {
